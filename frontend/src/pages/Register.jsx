...
export default function Register() {
  ...
  const { register } = useAuth(); // 2. Obtenha a função de registro do contexto
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await register(userData); // 3. Use a função de registro do contexto
      ...
      <div className="w-full max-w-md p-8 space-y-6 bg-white shadow-2xl rounded-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <Input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Seu nome" required disabled={isLoading}/>
          </div>
          <div>
          ...
          <div>
            ...
            <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Crie uma senha forte" required disabled={isLoading}/>
          </div>
